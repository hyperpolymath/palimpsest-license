...
**Status**: ✅ **100% compliance** (Gold standard)
...
**Status**: 🔄 **Planned** (Q2 2026 implementation)
...
┌─────────────────────────────────────────┐
│          Deno KV Backend                │
│  (CRDT Storage + Replication)           │
└─────────────────────────────────────────┘
                    ▲
                    │
┌───────────────────┼───────────────────┐
│                   │                   │
│  License       Metadata           DAO │
│  Metadata      Validator     Governance│
│  (LWW-Set)     (OR-Set)       (OR-Set) │
└───────────────────────────────────────┘
----
...
**Status**: 🔄 **Partial** (Podman planned Q2 2026)
...
- ✅ **SPDX headers**: Every file's license verified
- ✅ **Git signatures**: Commits can be GPG-signed (recommended for maintainers)
- 🔄 **Cryptographic provenance**: `.well-known/provenance.json` (in progress)
- 🔄 **Service mesh**: mTLS between services (planned Q3 2026)
...
**Status**: 🔄 **Planned** (Chainguard Wolfi Q2 2026)
...
----
$ just audit-licence
🔍 Auditing SPDX license identifiers...
✅ All Haskell files have SPDX headers
✅ All ReScript files have SPDX headers
✅ All JavaScript files have SPDX headers
...
📊 Analysing npm dependencies...
📊 Analysing Haskell dependencies...
📦 Consolidating into CycloneDX format...
...
|Dependency pinning
|✅ package-lock.json, cabal.project.freeze
|100%
|===
...
**Status**: 🔄 **Partial** (QUIC planned, DNSSEC planned)
...
**Status**: 🔄 **Planned** (Q2 2026 for deployed services)
...
**Status**: 🔄 **Planned** (Q2 2026 for palimpsest-license.org domain)
...
**Current**: ❌ **No analytics** (most private option)

**Future**: 🔄 **Plausible Analytics** (Q3 2026, if needed)
...
- ✅ Storage limitation (Git history is operational, not marketing)

**Data subject rights**:

- ✅ Right to access (Git commits are public)
- ✅ Right to erasure (GitHub account deletion removes personal data)
- ✅ Right to portability (Git history exportable)
...
**Status**: 🔄 **Partial** (Documentation project has limited runtime)
...
- ✅ Documentation viewable offline (static files)
- ✅ Validators run offline (no network required)
...
**Status**: 🔄 **Partial** (CRDTs planned, supervision planned)
...
Conflict resolution (LWW-Set):
  Latest timestamp wins → version = "v0.4" (timestamp: 1234567895)
...
[source]
----
Process crashes → Supervisor detects → Restart process → Service restored

Example:
  MetadataStore crashes (bug in query)
  → Supervisor restarts MetadataStore
  → State recovered from CRDT store
  → Service continues
----
...
- Disk space >90% → Trigger cleanup
- Memory >90% → Garbage collection + restart
...
**Status**: ✅ **85% compliance** (SBOM implemented, attestations planned)
...
{
  "dependencies": {
    "prettier": "^3.0.0",  // ❌ Caret range
    "eslint": "~8.50.0"    // ❌ Tilde range
  }
}
----
...
1. **Security vulnerability reported** → Acknowledge within 24 hours
